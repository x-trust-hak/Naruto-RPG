    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            const text =
                ...
                m.message.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson?.replace(/.*"id":"([^"]+)".*/, '$1') ||
                '';
            ...
            console.log(`[ROUTE] from=${normalizedFrom} sender=${senderJid} pushName=${pushName} isGroup=${isGroup} fromMe=${m.key.fromMe} text=${cleanText}`);
            ...
            await require('./case')(conn, normalizedFrom, senderJid, cleanText, phoneNumber, pushName);
