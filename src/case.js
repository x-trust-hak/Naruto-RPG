    ...
    try {
        ...
        switch (command) {
            ...
            case 'profile': {
                ...
                const profileMsg =
                    `📜 *SHINOBI PROFILE* 📜\n\n` +
                    `${user.isAkatsuki ? `🌑 *AKATSUKI MEMBER* ${getRing(user.phoneId) ? `— ${getRing(user.phoneId).color} Ring: ${getRing(user.phoneId).name}` : ''}\n` : ''}` +
                    ...
                    `${user.equippedWeapon ? `🗡️ *Weapon:* ${getWeapon(user.equippedWeapon)?.name || user.equippedWeapon} — _!weaponjutsu in battle_\n` : ''}` +
            ...
            case 'fight': {
                ...
                const screen =
                    ...
                    `${wepOnBattle ? `🗡️ *${wepOnBattle.emoji} ${wepOnBattle.name}* equipped — !weaponjutsu to use ${wepOnBattle.weaponJutsu.name}\n` : ''}\n` +
                    `🥋 *Your jutsus:*\n${moveList}` +
                    `  *0.* Basic Strike (taijutsu, unlimited)\n\n` +
                    `_Reply *!use [number]* to strike, *!weaponjutsu* for weapon move, *!item [name]* for items, or *!flee* to retreat._`;
