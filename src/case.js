...
const { getCurrentSeason, getTier, getTierProgress, spBar, buildSeasonBoard, buildArenaCard, processMatchResult, endSeason, hallOfFameText, SP_GAIN_WIN, SP_LOSS_LOSE, SEASON_END_REWARDS } = require('./arenaSeason');
const { VILLAGE_EMBLEMS, activeWars, pendingWarRequests, postWarBonuses, WAR_COST_RYO, WAR_KAGE_COST, WAR_PVP_RYO_BONUS, WIN_RYO_MULT, getEmblem, getVillageWar, getWarBetween, areAtWar, getWarBonus, warRyoMultiplier, sendWarRequest, acceptWarRequest, declineWarRequest, getPendingRequest, processWarKill, endWar, warStatusText, warResultText, warHistoryText } = require('./villageWar');
const { TRAINING_OPTIONS, getOption, startTraining, getTrainingStatus, collectTraining, trainingGroundText, trainingProgressBar, trainingStatusText, collectionResultText } = require('./trainingGround');
const { getSynergy, calcTrainingCost, generateSecretTechnique, startTechniqueTraining, getTechniqueTrainingStatus, techniqueGuideText, trainingStartedText, trainingStatusText: techStatusText, collectedText } = require('./secretTechnique');
const { DAILY_GIFT_RYO_CAP, MIN_GIFT_RYO, MIN_LEVEL_TO_GIFT, getRemainingGiftCap, validateRyoGift, validateItemGift, executeRyoGift, executeItemGift, ryoGiftConfirmText, itemGiftConfirmText, ryoReceivedText, itemReceivedText, giftHistoryText } = require('./giftSystem');
const { pvpKillAnnouncement, raidKillAnnouncement, bountyKillAnnouncement, checkMilestone, milestoneAnnouncement, championAnnouncement } = require('./killAnnounce');
const { REST_HP_PER_HOUR, startRest, getRestStatus, collectRest, calcTreatmentCost, calcEmergencyGemCost, executeTreatment, hospitalText, restProgressText, collectRestText } = require('./hospital');
...
async function finishPveWin(conn, from, user, fight, log) {
    ...
    let ryo = Math.floor((Math.random() * (r.maxRyo - r.minRyo + 1)) + r.minRyo) * ryoMultiplier() * warRyoMultiplier(user.village || '');
    ryo = Math.round(ryo);
    ...
    await conn.sendMessage(from, prepareImagePayload(getBattleImage(user.character), reply));

    // ── Milestone check (level / lifetime wins from PvE) ──────────────
    const levelMs = checkMilestone('level', levelBefore, user.level);
    const winsMs  = checkMilestone('wins', winsBefore, user.wins || 0);
    if (levelMs) {
        await conn.sendMessage(from, { text: milestoneAnnouncement(user.username, 'level', levelMs) });
    } else if (winsMs) {
        await conn.sendMessage(from, { text: milestoneAnnouncement(user.username, 'wins', winsMs) });
    }
}
        ...
        switch (command) {
            ...
            case 'start': {
                ...
                const welcomeMsg =
                    ...
                    `_!guide -- full walkthrough  |  !menu -- command list  |  !shop -- get more characters_`;
            ...
            case 'balance': {
                ...
                const warBonusW = getWarBonus(user.village || '');
                const tgStatus = getTrainingStatus(user);
                const restStatusW = getRestStatus(user);
                const walletMsg =
                    ...
                    `${warBonusW ? `⚔️ *WAR BONUS ACTIVE:* ×${warBonusW.ryoMult} Ryo (expires in ${Math.ceil((warBonusW.endTime - Date.now()) / 3600000)}h)\n` : ''}` +
                    `${getVillageWar(user.village || '') ? `🏯 *YOUR VILLAGE IS AT WAR!* — !war for status\n` : ''}` +
                    `${tgStatus.training ? `${tgStatus.opt.emoji} *Training:* ${tgStatus.complete ? 'Complete! !trainingground collect' : `${tgStatus.progress}% done`}\n` : ''}` +
                    `${restStatusW.resting ? `🛌 *Resting:* ${restStatusW.isFull ? 'Fully healed! !rest collect' : `${restStatusW.currentHp}/${restStatusW.maxHp} HP`}\n` : ''}` +
                    `${(user.hp.current < user.hp.max && !restStatusW.resting) ? `🏥 *Injured!* — !hospital for healing options\n` : ''}` +
            ...
            case 'battlestats': {
                ...
                const recordMsg =
                    ...
                    `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                    `⚔️ *War Kills:* ${user.warKills || 0} | 🏯 VP Contributed: ${(user.warVpContributed||0).toLocaleString()}\n` +
            ...
            case 'profile': {
                ...
                const profileMsg =
                    ...
                    // ── Combat record
                    `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                    `⚔️ *Record:* ${user.wins || 0}W / ${user.losses || 0}L${user.winStreak > 1 ? ` 🔥${user.winStreak} streak` : ''} | 🏅 ${user.rankPoints || 0} RP\n` +
                    (() => { const tier = getTier(user.seasonSp || 0); return `${tier.color} *Season Tier:* ${tier.emoji} ${tier.name} | 🌟 ${user.seasonSp || 0} SP (${user.seasonWins||0}W/${user.seasonLosses||0}L this season)\n`; })() +
                    `${getVillageWar(user.village || '') ? `⚔️ *VILLAGE AT WAR!* — ${getVillageWar(user.village).attacker} vs ${getVillageWar(user.village).defender}\n` : ''}` +
                    `${getWarBonus(user.village || '') ? `🏆 *War Victory Bonus:* ×${getWarBonus(user.village).ryoMult} Ryo active!\n` : ''}` +
                    ...
                    `${user.secretTechnique ? `🌀 *${user.secretTechnique.name}* | !use secret\n` +
                                   `   ${user.secretTechnique.damage} dmg | ${user.secretTechnique.cost} chakra\n` : ''}` +
                    ...
                    (!bijuuChar && !dojutsuD && !wepData && !user.secretTechnique && !user.isAkatsuki ? `_No special abilities yet — !weaponshop | !dojutsu | !technique | !akatsuki_\n` : '') +
                    `\n` +
                    `_!jutsus — moves | !bag — items | !guide — full walkthrough_`;
            ...
            case 'jutsu': {
                ...
                if (user.secretTechnique) {
                    const st = user.secretTechnique;
                    jutsuList += `\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
                    jutsuList += `🌀 *SECRET TECHNIQUE* 🌀\n`;
                    jutsuList += `*${st.name}*${st.nature ? ` (${natureTag(st.nature)})` : ''}\n`;
                    jutsuList += `   💧 Cost: ${st.cost} Chakra | ⟳ ${st.pp} PP\n`;
                    jutsuList += `   📖 ${st.desc}\n`;
                    jutsuList += `   ⚡ _Used with !use secret in battle_\n`;
                } else {
                    jutsuList += `\n\n_🌀 No Secret Technique yet — !technique to train one!_`;
                }
            ...
            case 'fight': {
                ...
                user.equippedJutsus.forEach((id, i) => {
                    const j = char.jutsus.find(x => x.id === id);
                    if (j) moveList += `   *${i + 1}.* ${j.name}${j.buff ? ' 💫' : ''}${j.nature ? ` ${natureTag(j.nature)}` : ''}\n        💧${j.cost} chakra · ⟳${jutsuPp[id]}/${j.pp ?? '∞'} uses\n`;
                });
                ...
                // Build special abilities block — every special action clearly listed
                let abilityHints = '';
                if (user.secretTechnique) {
                    abilityHints += `   🌀 *${user.secretTechnique.name}*\n        !use secret · 💧${user.secretTechnique.cost} chakra\n`;
                }
                if (wepOnBattle) {
                    abilityHints += `   🗡️ *${wepOnBattle.name}*\n        !weaponjutsu (${wepOnBattle.weaponJutsu.name}, ${wepOnBattle.weaponJutsu.damage} dmg)\n`;
                }
                if (dojutsuOnBattle) {
                    abilityHints += `   ${dojutsuOnBattle.emoji} *${dojutsuOnBattle.name}* active\n        !dojutsu [1/2/3]${dojutsuOnBattle.eye === 'sharingan' ? ' · !dojutsu copy' : ''}\n`;
                }
                if (bijuuOnBattle) {
                    abilityHints += `   ${bijuuOnBattle.emoji} *${bijuuOnBattle.name}* — Tailed Beast\n        !beast (Chakra Mode)${hasBijuuModeAvail ? ' · !bijuumode (Stage 3)' : ''}\n`;
                }
                if (user.isAkatsuki) {
                ...
                const screen =
                    ...
                    `⚔️${ps.attack}  🛡️${ps.defense}  💨${ps.speed}  💥${ps.crit}%\n\n` +
                    `━━━━━━━━━━━━━━━━━━━━\n` +
                    `🥋 *YOUR JUTSUS*\n${moveList}` +
                    `   *0.* Basic Strike\n        Free taijutsu, unlimited uses\n` +
                    (abilityHints ? `\n━━━━━━━━━━━━━━━━━━━━\n✨ *SPECIAL ABILITIES*\n${abilityHints}` : '') +
                    `\n━━━━━━━━━━━━━━━━━━━━\n` +
                    `_!use [number] to attack · !flee to escape_`;
                ...
                // Quick-action buttons — covers jutsu + every special action type the player has
                const btnRows = [];
                const firstJutsuId = user.equippedJutsus[0];
                const firstJutsu = firstJutsuId ? char.jutsus.find(x => x.id === firstJutsuId) : null;
                if (firstJutsu) btnRows.push({ label: `🌀 ${firstJutsu.name}`, id: '!use 1' });
                if (user.secretTechnique) btnRows.push({ label: `🌀 ${user.secretTechnique.name}`, id: '!use secret' });
                if (wepOnBattle) btnRows.push({ label: `🗡️ ${wepOnBattle.weaponJutsu.name}`, id: '!weaponjutsu' });
                else if (bijuuOnBattle) btnRows.push({ label: `${bijuuOnBattle.emoji} Chakra Mode`, id: '!beast' });
                else if (dojutsuOnBattle) btnRows.push({ label: `${dojutsuOnBattle.emoji} ${dojutsuOnBattle.name}`, id: '!dojutsu' });
                btnRows.push({ label: '💨 Flee', id: '!flee' });
            ...
            case 'attack': {
                ...
                if (fight.type === 'pvp') {
                    ...
                    if (p1Dead || p2Dead) {
                        ...
                        // Season SP + RP update
                        const seasonResult = processMatchResult(winner, loser);

                        const winnerLevelBefore = winner.level;
                        const winnerWinsBefore  = winner.wins || 0;
                        ...
                        // Village honor bonus vs Akatsuki
                        if (loser.isAkatsuki && !winner.isAkatsuki) {
                            winner.ryo += 2000;
                            bountyLog += `\n🏯 *Village Honor Bonus:* +2,000 Ryo for defeating an Akatsuki member!`;
                        }

                        // Village War — check if combatants are from opposing war villages
                        let warLog = '';
                        if (winner.village && loser.village && winner.village !== loser.village &&
                            !winner.isAkatsuki && !loser.isAkatsuki) {
                            const war = getWarBetween(winner.village, loser.village);
                            if (war) {
                                const warResult = processWarKill(war, winner.village, loser.village);
                                if (warResult) {
                                    winner.ryo += warResult.ryoBonus;
                                    winner.warKills = (winner.warKills || 0) + 1;
                                    winner.warVpContributed = (winner.warVpContributed || 0) + warResult.vpGained;
                                    const atk = getEmblem(war.attacker);
                                    const def = getEmblem(war.defender);
                                    warLog = `\n⚔️ *WAR KILL!* +${warResult.ryoBonus.toLocaleString()} Ryo war bonus!\n` +
                                             `${atk.emoji} ${war.attacker}: ${warResult.attackerVP} VP | ${def.emoji} ${war.defender}: ${warResult.defenderVP} VP`;
                                }
                            }
                        }
                        ...
                        // Build tier/promotion info
                        const winTierStr = `${seasonResult.newTier.emoji} ${seasonResult.newTier.name}`;
                        const promoStr   = seasonResult.promoted
                            ? `\n🎉 *TIER UP! ${seasonResult.prevTier.name} → ${seasonResult.newTier.name}!*`
                            : '';

                        log += `\n🏆 *${winner.username} WINS!*\n` +
                               `💰 +800 Ryo | 📈 +120 XP\n` +
                               `🏅 +${seasonResult.rpGained} RP | 🌟 +${SP_GAIN_WIN} SP → ${winner.seasonSp} SP (${winTierStr})${promoStr}\n` +
                               `📉 ${loser.username}: -${SP_LOSS_LOSE} SP | -${seasonResult.rpLost} RP`;
                        log += bountyLog;
                        log += warLog;
                        if (rewards.length) log += `\n🎉 ${winner.username} leveled up to Lv.${winner.level}!`;
                        log += `\n\n_!arena — your season ranking | !season — leaderboard_`;
                        await conn.sendMessage(fight.from, { text: log });

                        // ── Kill Announcement — hype message for the win ──
                        const killMsg = pvpKillAnnouncement({
                            winnerName: winner.username,
                            loserName: loser.username,
                            winnerStreak: winner.winStreak || 0,
                            isWarKill: !!warLog,
                            warVillages: winner.village && loser.village ? `${winner.village} vs ${loser.village}` : null,
                            isBountyKill: !!loserBounty,
                            bountyAmount: loserBounty?.amount,
                            isAkatsukiKill: loser.isAkatsuki && !winner.isAkatsuki,
                            tierUp: seasonResult.promoted,
                            newTier: seasonResult.newTier?.name,
                        });
                        await conn.sendMessage(fight.from, { text: killMsg });

                        // ── Milestone check (level / lifetime wins) ────────
                        const levelMilestone = checkMilestone('level', winnerLevelBefore, winner.level);
                        const winMilestone   = checkMilestone('wins', winnerWinsBefore, winner.wins || 0);
                        if (levelMilestone) {
                            await conn.sendMessage(fight.from, { text: milestoneAnnouncement(winner.username, 'level', levelMilestone) });
                        } else if (winMilestone) {
                            await conn.sendMessage(fight.from, { text: milestoneAnnouncement(winner.username, 'wins', winMilestone) });
                        }
                        return;
                    }
                ...
                // !use with no argument -> show live full action menu, not just jutsus
                if (!args[0]) {
                    const wepStatus = user.equippedWeapon ? getWeapon(user.equippedWeapon) : null;
                    const bijuuStatus = getBijuu(user.character);
                    const dojutsuStatus = hasDojutsu(user.character) ? require('./dojutsu').DOJUTSU[user.character] : null;
                    const hasBijuuModeStatus = bijuuStatus && BIJUU_MODE[user.character];

                    let statusList = `🥋  *ROUND ${fight.round} — YOUR MOVES* 🥋\n━━━━━━━━━━━━━━━━━━━━\n\n`;
                    user.equippedJutsus.forEach((id, i) => {
                        const j = char.jutsus.find(x => x.id === id);
                        if (!j) return;
                        const left = fight.jutsuPp[id] ?? 0;
                        statusList += `   *${i + 1}.* ${j.name}${j.buff ? ' 💫' : ''}${j.nature ? ` ${natureTag(j.nature)}` : ''}\n        💧${j.cost} · ⟳${left}/${j.pp ?? '∞'}${left <= 0 ? ' ❌ EMPTY' : ''}\n        _!use ${i + 1}_\n`;
                    });
                    statusList += `   *0.* Basic Strike\n        Free, unlimited\n        _!use 0_\n`;

                    let specialList = '';
                    if (user.secretTechnique) {
                        const stPp = fight.secretPp ?? user.secretTechnique.pp;
                        specialList += `   🌀 *${user.secretTechnique.name}*\n        💧${user.secretTechnique.cost} · ⟳${stPp}/${user.secretTechnique.pp}${stPp <= 0 ? ' ❌ EMPTY' : ''}\n        _!use secret_\n`;
                    }
                    if (wepStatus) {
                        specialList += `   🗡️ *${wepStatus.weaponJutsu.name}* (${wepStatus.name})\n        ${wepStatus.weaponJutsu.damage} dmg\n        _!weaponjutsu_\n`;
                    }
                    if (dojutsuStatus) {
                        specialList += `   ${dojutsuStatus.emoji} *${dojutsuStatus.name}*\n        Active techniques\n        _!dojutsu_\n`;
                    }
                    if (bijuuStatus) {
                        specialList += `   ${bijuuStatus.emoji} *${bijuuStatus.name}* — Tailed Beast\n        ${fight.bijuuState?.chakraMode ? 'Chakra Mode ACTIVE' : fight.bijuuState?.bijuuMode ? 'Bijuu Mode ACTIVE' : `!beast for Chakra Mode${hasBijuuModeStatus ? ' · !bijuumode for Stage 3' : ''}`}\n        _!beast / !bijuumode_\n`;
                    }
                    ...
                    statusList += `\n━━━━━━━━━━━━━━━━━━━━\n_!use [number] to act · !flee to escape_`;
                    return await conn.sendMessage(from, { text: statusList });
                }

                const isBasic = args[0] === '0' || args[0].toLowerCase() === 'basic';
                const isSecret = args[0].toLowerCase() === 'secret' || args[0].toLowerCase() === 's';
                const idx = parseInt(args[0]) - 1;
                const jutsuId = (!isBasic && !isSecret) ? user.equippedJutsus[idx] : null;
                const jutsu = jutsuId ? char.jutsus.find(j => j.id === jutsuId) : null;
                const secretTech = isSecret ? user.secretTechnique : null;

                if (isSecret && !secretTech) {
                    return await conn.sendMessage(from, { text: `❌ You don't have a Secret Technique yet!\n\n_!technique — train one at the Training Ground_` });
                }
                if (!isBasic && !isSecret && !jutsu) {
                    return await conn.sendMessage(from, { text: `❌ Invalid move. You have ${user.equippedJutsus.length} jutsu(s). Use !use 1, !use secret, or !use 0 for Basic Strike.` });
                }
                if (!isBasic && !isSecret && (fight.jutsuPp[jutsu.id] ?? 0) <= 0) {
                ...
                if (!isBasic && !isSecret && user.chakra.current < jutsu.cost) {
                    return await conn.sendMessage(from, { text: `❌ Not enough chakra for *${jutsu.name}* (need ${jutsu.cost}, have ${user.chakra.current}).\nTry a cheaper jutsu, !use 0 for Basic Strike, or !item soldier pill.` });
                }
                if (isSecret) {
                    if (!fight.secretPp) fight.secretPp = secretTech.pp;
                    if (fight.secretPp <= 0) {
                        return await conn.sendMessage(from, { text: `❌ *${secretTech.name}* is out of PP for this battle!` });
                    }
                    if (user.chakra.current < secretTech.cost) {
                        return await conn.sendMessage(from, { text: `❌ Not enough chakra for *${secretTech.name}* (need ${secretTech.cost}, have ${user.chakra.current}).` });
                    }
                }
                ...
                const moveName = isBasic ? 'Basic Strike' : (isSecret ? secretTech.name : jutsu.name);
                const moveDamage = isBasic ? 50 : (isSecret ? secretTech.damage : jutsu.damage);
                const moveNature = isBasic ? null : (isSecret ? secretTech.nature : jutsu.nature);
                const moveBuff = isBasic ? null : (isSecret ? null : jutsu.buff);

                if (isSecret) {
                    user.chakra.current -= secretTech.cost;
                    fight.secretPp -= 1;
                } else if (!isBasic) {
                    user.chakra.current -= jutsu.cost;
                    fight.jutsuPp[jutsu.id] -= 1;
                }
                let log = isSecret ? `🌀 *Secret Technique unleashed!*\n` : '';
                ...
                if (!isBasic && !isSecret && fight.jutsuPp[jutsu.id] === 0) log += `_(${jutsu.name} is now out of PP!)_\n`;
                if (isSecret && fight.secretPp === 0) log += `_(${secretTech.name} is now out of PP!)_\n`;
                ...
                const screen =
                    ...
                    `━━━━━━━━━━━━━━━━━━━━\n` +
                    `_Round ${fight.round} · !use for your move list · !flee to escape_`;
            ...
            case 'run': {
                ...
                if (fleeFight.type === 'pvp') {
                    ...
                    if (fleeUser) {
                        fleeUser.losses = (fleeUser.losses || 0) + 1;
                        fleeUser.winStreak = 0;
                        fleeUser.seasonSp = Math.max(0, (fleeUser.seasonSp || 0) - SP_LOSS_LOSE);
                        fleeUser.seasonLosses = (fleeUser.seasonLosses || 0) + 1;
                        fleeUser.rankPoints = Math.max(0, (fleeUser.rankPoints || 0) - 15);
                        await fleeUser.save();
                    }
                    if (oppUser) {
                        oppUser.wins = (oppUser.wins || 0) + 1;
                        oppUser.winStreak = (oppUser.winStreak || 0) + 1;
                        oppUser.seasonSp = (oppUser.seasonSp || 0) + SP_GAIN_WIN;
                        oppUser.seasonWins = (oppUser.seasonWins || 0) + 1;
                        oppUser.rankPoints = (oppUser.rankPoints || 0) + 25;
                        oppUser.ryo += 800;
                        await oppUser.save();
                    }
            ...
            case 'leaderboard': {
                ...
                if (sortBy === 'xp' || sortBy === 'level') {
                    ...
                    let board = `🏆 *XP LEADERBOARD* 🏆\n\n`;
                    topPlayers.forEach((p, i) => {
                        const char = getCharacter(p.character);
                        const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`;
                        board += `${medal} *${p.username}* ${p.isKage ? '👑' : ''}\n`;
                        board += `   ${char.emoji} Lv.${p.level} | ${(p.totalXp||0).toLocaleString()} XP | ${p.village}\n\n`;
                    });
                    board += `_!top — season board | !top xp — XP board_`;
                    return await conn.sendMessage(from, { text: board });
                }

                // Default: Season leaderboard
                const board = await buildSeasonBoard(User);
                return await conn.sendMessage(from, prepareImagePayload('welcome', board));
            }
                        ...
                        sections: [
                            {
                                title: '━━━  🚀 GET STARTED  ━━━',
                                rows: [
                                    { id: '!guide',     title: '!guide',             description: '📖 Full walkthrough -- read this if you\'re lost!' },
                            ...
                            },
                            {
                                ...
                                rows: [
                                    ...
                                    { id: '!technique',  title: '!technique',          description: '🌀 Train a permanent Secret Technique by fusing 2 jutsus' },
                                ]
                            },
                            {
                                ...
                                rows: [
                                    ...
                                    { id: '!train',     title: '!train',              description: '💪 Quick instant XP grind (uses chakra)' },
                                    { id: '!trainingground', title: '!trainingground',   description: '🏯 Passive idle training — send your ninja off for hours' },
                                    { id: '!hospital',  title: '!hospital',           description: '🏥 Pay Ryo or Gems for instant full HP heal' },
                                    { id: '!rest',      title: '!rest',               description: '🛌 Free passive HP recovery over time (200 HP/hour)' },
                                    ...
                                    { id: '!arena',     title: '!arena',              description: '🌟 Your personal season rank, SP, tier & progress bar' },
                                    { id: '!season',    title: '!season',             description: '📅 Season leaderboard & time remaining' },
                                    { id: '!top',       title: '!top',                description: '🏆 Season leaderboard | !top xp for XP board' },
                                    { id: '!tiers',     title: '!tiers',              description: '⚔️ All arena tiers, SP requirements & season rewards' },
                                ]
                            },
                        ]
                    }));
                ...
                return await conn.sendMessage(from, listMessage({
                    ...
                    sections: [
                        ...
                        },
                        {
                            title: '━━━  ⚔️ VILLAGE WAR  ━━━',
                            rows: [
                                { id: '!war',              title: '!war',               description: '⚔️ Active war status, or pending request for your village' },
                                { id: '!war declare',      title: '!war declare [name]',description: '🚨 Send a war request to a village (Kage only)' },
                                { id: '!war accept',       title: '!war accept',        description: '✅ Accept an incoming war request (defending Kage only)' },
                                { id: '!war decline',      title: '!war decline',       description: '❌ Refuse an incoming war request (defending Kage only)' },
                                { id: '!war surrender',    title: '!war surrender',     description: '🏳️ Surrender an active war (Kage only)' },
                                { id: '!war history',      title: '!war history',       description: '📜 Past war results between villages' },
                            ]
                        },
                        {
                        ...
                        },
                        {
                            title: '━━━  🎁 GIFTS & EXTRAS  ━━━',
                            rows: [
                                { id: '!gift',           title: '!gift [name] [amt]', description: '🎁 Send Ryo to another player' },
                                { id: '!gift item',      title: '!gift item [name] [item]', description: '📦 Send an item from your bag to another player' },
                                { id: '!gift history',   title: '!gift history',      description: '📜 Your recent gift activity' },
                                { id: '!killfeed',       title: '!killfeed',          description: '📣 What gets announced automatically (wins, raids, milestones)' },
            ...
            case 'raid': {
                ...
                if (subCmd === 'attack' || subCmd === 'hit' || subCmd === 'strike') {
                    ...
                    if (raid.hp <= 0) {
                        ...
                        await user.save();
                        await conn.sendMessage(from, prepareImagePayload('akatsuki', deathMsg));

                        // ── Kill Announcement for raid boss defeat ─────────
                        const raidKillMsg = raidKillAnnouncement({
                            killerName: user.username,
                            bossName: raid.boss.name,
                            totalParticipants: allRewards.length,
                            isCrit: crit,
                        });
                        return await conn.sendMessage(from, { text: raidKillMsg });
                    }
            ...
            case 'smission': {
                ...
                // Success
                const ryoBase = Math.floor(Math.random() * (mission.maxRyo - mission.minRyo + 1)) + mission.minRyo;
                const ryo = Math.round(ryoBase * warRyoMultiplier(user.village || ''));
                let gems = 0;
            ...
            case 'myrank': {
                ...
                const card = await buildArenaCard(user, User);
                return await conn.sendMessage(from, prepareImagePayload('welcome', card));
            }

            // ════════════════════════════════════════════════════════════════
            // SEASON — FULL LEADERBOARD
            // ════════════════════════════════════════════════════════════════
            case 'season':
            case 'seasons': {
                const sub = (args[0] || '').toLowerCase();
                ...
                // Hall of Fame
                if (sub === 'hof' || sub === 'halloffame' || sub === 'history') {
                    return await conn.sendMessage(from, { text: hallOfFameText() });
                }

                // Season info
                if (sub === 'info') {
                    const season = getCurrentSeason();
                    const totalInSeason = await User.countDocuments({
                        registrationStep: 'COMPLETED', seasonSp: { $gt: 0 }
                    });
                    return await conn.sendMessage(from, {
                        text: `📅 *CURRENT SEASON*\n\n` +
                              `🌟 *${season.name}*\n` +
                              `📅 Started: ${season.start.toLocaleDateString()}\n` +
                              `⏰ Ends: ${season.end.toLocaleDateString()} (${season.daysLeft} days left)\n\n` +
                              `👥 Active players: ${totalInSeason}\n\n` +
                              `🏆 *Top 3 get:*\n` +
                              `🥇 ${SEASON_END_REWARDS.rank1.ryo.toLocaleString()} Ryo | ${SEASON_END_REWARDS.rank1.gems} 💎 | +${SEASON_END_REWARDS.rank1.hpBonus} Max HP\n` +
                              `🥈 ${SEASON_END_REWARDS.rank2.ryo.toLocaleString()} Ryo | ${SEASON_END_REWARDS.rank2.gems} 💎\n` +
                              `🥉 ${SEASON_END_REWARDS.rank3.ryo.toLocaleString()} Ryo | ${SEASON_END_REWARDS.rank3.gems} 💎\n\n` +
                              `🌟 *+${SP_GAIN_WIN} SP per win | -${SP_LOSS_LOSE} SP per loss*\n\n` +
                              `_!arena — your personal ranking_\n` +
                              `_!top — full season leaderboard_\n` +
                              `_!duel [name] — earn SP!_`
                    });
                }
                ...
                if (sub === 'end' || sub === 'reset') {
                    if (!user?.isAdmin) {
                        return await conn.sendMessage(from, { text: '❌ Admin only command.' });
                    }
                    const { announcements, season } = await endSeason(User, conn, from);
                    let msg = `🏆 *SEASON OVER — ${season.name}*\n\n`;
                    msg += `*Champions:*\n${announcements.join('\n')}\n\n`;
                    msg += `Season stats reset. New season begins now!\n`;
                    msg += `_All players earned tier-based participation rewards._`;
                    return await conn.sendMessage(from, prepareImagePayload('welcome', msg));
                }
            ...
            case 'tiers':
            case 'tierlist': {
                const { SEASON_TIERS } = require('./arenaSeason');
                let txt = `⚔️ *ARENA SEASON TIERS* ⚔️\n\n`;
                txt += `_Win PvP duels to earn SP and climb the ranks!_\n\n`;
                for (const t of SEASON_TIERS) {
                    const max = t.maxSp === Infinity ? '∞' : t.maxSp.toLocaleString();
                    txt += `${t.color} ${t.emoji} *${t.name}*\n`;
                    txt += `   ${t.minSp.toLocaleString()}–${max} SP\n`;
                    txt += `   Win: +${t.rpGain} RP | Loss: -${t.rpLoss} RP\n`;
                    const rew = SEASON_END_REWARDS.tiers[t.name];
                    if (rew) txt += `   Season reward: 💰${rew.ryo.toLocaleString()} | 📈${rew.xp.toLocaleString()} XP${rew.gems ? ` | 💎${rew.gems}` : ''}\n`;
                    txt += `\n`;
                }
                txt += `_+${SP_GAIN_WIN} SP per PvP win | -${SP_LOSS_LOSE} SP per loss_\n`;
                txt += `_!arena — your current tier | !duel [name] — fight!_`;
                return await conn.sendMessage(from, { text: txt });
            }
            ...
            case 'villagewar': {
                ...
                if (!sub || sub === 'status' || sub === 'info') {
                    // Show war involving user's village first
                    const myWar = user.village && !user.isAkatsuki ? getVillageWar(user.village) : null;
                    if (myWar) {
                        return await conn.sendMessage(from, prepareImagePayload('akatsuki', warStatusText(myWar)));
                    }
                    // Show all active wars
                    const allWars = [...activeWars.values()].filter(w => w.status === 'active' && Date.now() < w.endTime);
                    if (!allWars.length) {
                        const bonus = user.village ? getWarBonus(user.village) : null;
                        return await conn.sendMessage(from, {
                            text: `🕊️ *PEACE TIME*\n\n_No wars are active right now._\n\n` +
                                  (bonus ? `⚔️ *Your village still has the war victory bonus:*\n×${bonus.ryoMult} Ryo — expires in ${Math.ceil((bonus.endTime - Date.now())/3600000)}h\n\n` : '') +
                                  `_Kage can declare war with !war declare [village]_\n` +
                                  `_!war history — past wars_`
                        });
                    }
                    let txt = `⚔️ *ACTIVE WARS*\n\n`;
                    for (const w of allWars) {
                        const atk = getEmblem(w.attacker);
                        const def = getEmblem(w.defender);
                        const msLeft = Math.max(0, w.endTime - Date.now());
                        const daysLeft = Math.floor(msLeft / 86400000);
                        txt += `${atk.emoji} *${w.attacker}* vs ${def.emoji} *${w.defender}*\n`;
                        txt += `   VP: ${w.attackerVP} – ${w.defenderVP} | ${daysLeft}d left\n\n`;
                    }
                    txt += `_!war [village] — detailed status for a specific war_`;
                    return await conn.sendMessage(from, { text: txt });
                }

                // ── !war [village name] — show specific war ────────────────
                const targetVillage = sub.charAt(0).toUpperCase() + sub.slice(1);
                if (!sub.startsWith('declare') && !sub.startsWith('end') && !sub.startsWith('hist') && !sub.startsWith('sur')) {
                    const specificWar = getVillageWar(targetVillage);
                    if (specificWar) {
                        return await conn.sendMessage(from, prepareImagePayload('akatsuki', warStatusText(specificWar)));
                    }
                }
                ...
                if (sub === 'declare') {
                    if (!user.isKage && !user.isAdmin) {
                        return await conn.sendMessage(from, {
                            text: `❌ *Only a Kage can declare war!*\n\n_Become Kage by reaching Lv.100 and winning the village election._\n_!kage — see your village's current Kage_`
                        });
                    }
                    if (user.isAkatsuki) {
                        return await conn.sendMessage(from, { text: `🌑 Akatsuki members cannot declare village wars. Use the bounty system instead.` });
                    }

                    const targetName = args[1] ? args[1].charAt(0).toUpperCase() + args[1].slice(1) : null;
                    if (!targetName) {
                        const { VILLAGES: vList } = require('./villageWar');
                        return await conn.sendMessage(from, {
                            text: `⚔️ *Declare war on which village?*\n\n` +
                                  `Usage: !war declare [village]\n\n` +
                                  `Villages: ${vList.join(', ')}\n\n` +
                                  `Example: !war declare Sand\n\n` +
                                  `⚠️ Cost: 💰 ${WAR_COST_RYO.toLocaleString()} Ryo (war treasury)`
                        });
                    }

                    if ((user.ryo || 0) < WAR_COST_RYO) {
                        return await conn.sendMessage(from, {
                            text: `💰 *Not enough Ryo to fund the war effort!*\n\nNeed: ${WAR_COST_RYO.toLocaleString()} Ryo\nYou have: ${(user.ryo||0).toLocaleString()} Ryo`
                        });
                    }

                    const { success, war, msg } = declareWar(user.village, targetName, user.username);
                    if (!success) return await conn.sendMessage(from, { text: msg });

                    user.ryo -= WAR_COST_RYO;
                    await user.save();
                    ...
                    const announcement =
                        `🚨 *WAR DECLARED!* 🚨\n\n` +
                        `${atk.emoji} *${war.attacker}* has declared war on ${def.emoji} *${war.defender}*!\n\n` +
                        `_"${user.username}, Kage of ${war.attacker}, has issued the declaration of war."_\n\n` +
                        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                        `⏰ *War lasts 7 days*\n` +
                        `💥 +${WAR_COST_RYO.toLocaleString()} Ryo per enemy kill in PvP\n` +
                        `🏆 Winning village gets ×${WIN_RYO_MULT} Ryo for 3 days\n\n` +
                        `_${war.attacker} ninja — challenge ${war.defender} players with !duel [name]!_\n` +
                        `_!war — live war status_`;
                    return await conn.sendMessage(from, prepareImagePayload('akatsuki', announcement));
                }

                // ── !war surrender ────────────────────────────────────────
                if (sub === 'surrender' || sub === 'sur') {
                    if (!user.isKage && !user.isAdmin) {
                        return await conn.sendMessage(from, { text: `❌ Only a Kage can surrender.` });
                    }
                    const myWar2 = user.village ? getVillageWar(user.village) : null;
                    if (!myWar2) {
                        return await conn.sendMessage(from, { text: `🕊️ Your village is not at war.` });
                    }
                    const result = endWar(myWar2);
                    const resText = warResultText(myWar2, result);
                    return await conn.sendMessage(from, prepareImagePayload('welcome',
                        `🏳️ *${user.village} HAS SURRENDERED!*\n\n${resText}`
                    ));
                }

                // ── !war end (admin) ──────────────────────────────────────
                if (sub === 'end' && user.isAdmin) {
                    const targetWar = args[1] ? getVillageWar(args[1].charAt(0).toUpperCase() + args[1].slice(1)) : null;
                    const warToEnd  = targetWar || (user.village ? getVillageWar(user.village) : null);
                    if (!warToEnd) return await conn.sendMessage(from, { text: `❌ No active war found.` });
                    const result = endWar(warToEnd);
                    return await conn.sendMessage(from, prepareImagePayload('welcome', warResultText(warToEnd, result)));
                }

                // ── !war history ──────────────────────────────────────────
                if (sub === 'history' || sub === 'hist' || sub === 'log') {
                    return await conn.sendMessage(from, { text: warHistoryText() });
                }

                return await conn.sendMessage(from, {
                    text: `⚔️ *VILLAGE WAR COMMANDS*\n\n` +
                          `!war — active war status\n` +
                          `!war declare [village] — declare war (Kage only)\n` +
                          `!war surrender — end the war (Kage only)\n` +
                          `!war history — past wars\n\n` +
                          `_Win PvP duels against enemy village ninja to earn VP for your village!_`
                });
            }
            ...
            // ════════════════════════════════════════════════════════════════
            // TRAINING GROUND — passive idle training
            // ════════════════════════════════════════════════════════════════
            case 'trainingground':
            case 'tg': {
                if (!user || user.registrationStep !== 'COMPLETED') {
                ...
                // ── !trainingground status ────────────────────────────────
                if (sub === 'status') {
                    const status = getTrainingStatus(user);
                    return await conn.sendMessage(from, prepareImagePayload(user.character, trainingStatusText(status)));
                }

                // ── !trainingground collect ───────────────────────────────
                if (sub === 'collect') {
                    const status = getTrainingStatus(user);
                    if (!status.training) {
                        return await conn.sendMessage(from, { text: `🏯 You're not training right now.\n\n_!trainingground — see options_` });
                    }
                    if (!status.complete) {
                        return await conn.sendMessage(from, {
                            text: `⏰ *Training isn't finished yet!*\n\n${trainingProgressBar(status.progress)}\n\n` +
                                  `_Use !trainingground cancel to collect early at a 40% penalty, or wait for it to finish._`
                        });
                    }

                    const result = collectTraining(user, false);
                    user.trainingSession = { active: false, optionId: '', startedAt: null, endsAt: null };
                    user.ryo = (user.ryo || 0) + result.ryo;
                    user.gems = (user.gems || 0) + result.gems;
                    ...
                    const { leveledUp, rewards } = await addXP(user, result.xp);
                    await user.save();

                    let resultMsg = collectionResultText(result, user.username);
                    if (leveledUp) {
                        rewards.forEach(r => {
                            resultMsg += `\n\n🎉 *LEVEL UP! Now Level ${r.level}!*\n💰 +${r.ryo} Ryo`;
                            if (r.gems > 0) resultMsg += ` | +${r.gems} 💎`;
                        });
                    }
                    return await conn.sendMessage(from, prepareImagePayload(user.character, resultMsg));
                }

                // ── !trainingground cancel — early collection ─────────────
                if (sub === 'cancel') {
                    const status2 = getTrainingStatus(user);
                    if (!status2.training) {
                        return await conn.sendMessage(from, { text: `🏯 You're not training right now.` });
                    }
                    if (status2.complete) {
                        return await conn.sendMessage(from, { text: `✅ Training is already complete! Use !trainingground collect instead — no penalty.` });
                    }

                    const result2 = collectTraining(user, true);
                    user.trainingSession = { active: false, optionId: '', startedAt: null, endsAt: null };
                    user.ryo = (user.ryo || 0) + result2.ryo;
                    const { leveledUp: lu2, rewards: rw2 } = await addXP(user, result2.xp);
                    await user.save();

                    let cancelMsg = collectionResultText(result2, user.username);
                    if (lu2) rw2.forEach(r => { cancelMsg += `\n\n🎉 *LEVEL UP! Now Level ${r.level}!*`; });
                    return await conn.sendMessage(from, { text: cancelMsg });
                }

                // ── !trainingground start [option] ────────────────────────
                if (sub === 'start') {
                    const optionId = (args[1] || '').toLowerCase();
                    if (!optionId) {
                        return await conn.sendMessage(from, {
                            text: `🏯 *Usage:* !trainingground start [option]\n\n` +
                                  `Options: ${TRAINING_OPTIONS.map(o => o.id).join(', ')}\n\n` +
                                  `_!trainingground — see full details_`
                        });
                    }
                    const { success, msg, session, opt } = startTraining(user, optionId);
                    if (!success) return await conn.sendMessage(from, { text: msg });

                    user.trainingSession = session;
                    await user.save();

                    return await conn.sendMessage(from, prepareImagePayload(user.character,
                        `${opt.emoji} *${opt.label} STARTED!*\n\n` +
                        `_${opt.desc}_\n\n` +
                        `⏱️ Duration: ${opt.hours} hour${opt.hours !== 1 ? 's' : ''}\n` +
                        `📈 Total: ${(opt.xpPerHour * opt.hours).toLocaleString()} XP\n` +
                        `💰 Total: ${(opt.ryoPerHour * opt.hours).toLocaleString()} Ryo\n\n` +
                        `_Come back later with !trainingground collect to claim your rewards!_\n` +
                        `_Check progress anytime: !trainingground status_`
                    ));
                }

                // ── Default: show training ground menu (auto-detect status) ─
                const currentStatus = getTrainingStatus(user);
                if (currentStatus.training) {
                    return await conn.sendMessage(from, prepareImagePayload(user.character, trainingStatusText(currentStatus)));
                }
                return await conn.sendMessage(from, prepareImagePayload(user.character, trainingGroundText(user)));
            }
            ...
            case 'send': {
                ...
                if (sub === 'history' || sub === 'log') {
                    return await conn.sendMessage(from, { text: giftHistoryText(user.username) });
                }

                // ── !gift item [player] [item name] [qty] ─────────────────
                if (sub === 'item') {
                    const targetName = args[1];
                    if (!targetName) {
                        return await conn.sendMessage(from, {
                            text: `🎁 *Usage:* !gift item [player] [item name] [qty]\n\n` +
                                  `Example: !gift item Sasuke99 soldier pill 2\n\n` +
                                  `_!bag — see your items first_`
                        });
                    }

                    // Parse: last arg might be qty, rest is item name
                    const restArgs = rawArgs.slice(2);
                    let qty = 1;
                    let itemNameRaw = restArgs.join(' ');
                    const lastArg = restArgs[restArgs.length - 1];
                    if (lastArg && /^\d+$/.test(lastArg)) {
                        qty = parseInt(lastArg);
                        itemNameRaw = restArgs.slice(0, -1).join(' ');
                    }

                    const item = findItem(itemNameRaw);
                    if (!item) {
                        return await conn.sendMessage(from, { text: `❌ Item not found: "${itemNameRaw}"\n\n_!bag — see your items_` });
                    }

                    const recipient = await findTargetUser(targetName);
                    if (!recipient) {
                        return await conn.sendMessage(from, { text: `❌ Player "*${targetName}*" not found.` });
                    }

                    const { valid, msg } = validateItemGift(user, recipient, item.id, qty);
                    if (!valid) return await conn.sendMessage(from, { text: msg });

                    executeItemGift(user, recipient, item.id, qty, item.name);
                    await Promise.all([user.save(), recipient.save()]);

                    await conn.sendMessage(from, { text: itemGiftConfirmText(user, recipient, item.name, qty) });

                    // Notify recipient if different chat / DM them if possible
                    try {
                        await conn.sendMessage(recipient.phoneId, { text: itemReceivedText(user, item.name, qty) });
                    } catch (_) { /* recipient may not be reachable directly, ignore */ }

                    return;
                }

                // ── !gift [player] [amount] — Ryo gift (default) ──────────
                const targetName2 = args[0];
                const amountRaw = args[1];

                if (!targetName2 || !amountRaw) {
                    return await conn.sendMessage(from, {
                        text: `🎁 *GIFT SYSTEM*\n\n` +
                              `!gift [player] [amount] — send Ryo\n` +
                              `!gift item [player] [item] [qty] — send an item\n` +
                              `!gift history — your recent gifts\n\n` +
                              `Example: !gift Naruto99 5000\n\n` +
                              `💰 Min gift: ${MIN_GIFT_RYO.toLocaleString()} Ryo\n` +
                              `📊 Daily cap: ${DAILY_GIFT_RYO_CAP.toLocaleString()} Ryo\n` +
                              `🔒 Requires Level ${MIN_LEVEL_TO_GIFT}+\n\n` +
                              `📊 Your remaining today: ${getRemainingGiftCap(user).toLocaleString()} Ryo`
                    });
                }

                const amount = parseInt(amountRaw.replace(/,/g, ''));
                if (isNaN(amount)) {
                    return await conn.sendMessage(from, { text: `❌ Invalid amount. Usage: !gift [player] [amount]` });
                }

                const recipient2 = await findTargetUser(targetName2);
                if (!recipient2) {
                    return await conn.sendMessage(from, { text: `❌ Player "*${targetName2}*" not found.\n\n_Check the spelling of their ninja username._` });
                }

                const { valid: ryoValid, msg: ryoMsg } = validateRyoGift(user, recipient2, amount);
                if (!ryoValid) return await conn.sendMessage(from, { text: ryoMsg });

                executeRyoGift(user, recipient2, amount);
                await Promise.all([user.save(), recipient2.save()]);

                await conn.sendMessage(from, { text: ryoGiftConfirmText(user, recipient2, amount) });

                // Notify recipient directly
                try {
                    await conn.sendMessage(recipient2.phoneId, { text: ryoReceivedText(user, amount, recipient2.ryo) });
                } catch (_) { /* recipient may not be reachable directly, ignore */ }
            ...
            case 'announcements': {
                return await conn.sendMessage(from, {
                    text: `📣 *KILL ANNOUNCEMENTS* 📣\n\n` +
                          `_Big moments get hyped up automatically in this group:_\n\n` +
                          `⚔️ PvP duel wins — with win streak callouts at 3/5/10/15/20\n` +
                          `💀 Boss raid killing blows\n` +
                          `🎯 Bounty collections\n` +
                          `🌑 Akatsuki member defeats\n` +
                          `⚔️ Village War kills\n` +
                          `🌟 Level milestones (25/50/75/100/150)\n` +
                          `🏆 Career win milestones (10/25/50/100/250)\n` +
                          `🎉 Season tier promotions\n\n` +
                          `_These post automatically — no command needed!_`
                });
            }


            // ════════════════════════════════════════════════════════════════
            // HOSPITAL — paid instant healing
            // ════════════════════════════════════════════════════════════════
            case 'hospital': {
                if (!user || user.registrationStep !== 'COMPLETED') {
                    return await conn.sendMessage(from, { text: '❌ *Not registered yet!*\n\nType *!start* to begin! 🥷' });
                ...
                if (activeFights.has(senderJid)) {
                    return await conn.sendMessage(from, { text: '⚔️ Can\'t visit the hospital mid-battle! Finish or !flee first.' });
                }

                const sub = (args[0] || '').toLowerCase();

                // ── !hospital heal — pay Ryo for instant full heal ─────────
                if (sub === 'heal' || sub === 'pay') {
                    const missing = user.hp.max - user.hp.current;
                    if (missing <= 0) {
                        return await conn.sendMessage(from, { text: `✅ You're already at full HP, ${user.username}!` });
                    }
                    const cost = calcTreatmentCost(user);
                    if ((user.ryo || 0) < cost) {
                        return await conn.sendMessage(from, {
                            text: `💰 *Not enough Ryo for treatment!*\n\nCost: ${cost.toLocaleString()} Ryo\nYou have: ${(user.ryo||0).toLocaleString()} Ryo\n\n_Try !rest instead — it's free, just takes time._`
                        });
                    }
                    user.ryo -= cost;
                    const healed = executeTreatment(user);
                    await user.save();
                    return await conn.sendMessage(from, {
                        text: `🏥 *TREATMENT COMPLETE!*\n\n💚 Fully healed! +${healed} HP\n❤️ ${user.hp.current}/${user.hp.max} HP\n💰 -${cost.toLocaleString()} Ryo\n\n_Back in fighting shape!_`
                    });
                }

                // ── !hospital surgery — pay Gems for instant full heal ─────
                if (sub === 'surgery' || sub === 'emergency') {
                    const missing2 = user.hp.max - user.hp.current;
                    if (missing2 <= 0) {
                        return await conn.sendMessage(from, { text: `✅ You're already at full HP, ${user.username}!` });
                    }
                    ...
                    if ((user.gems || 0) < gemCost) {
                        return await conn.sendMessage(from, {
                            text: `💎 *Not enough Gems!*\n\nCost: ${gemCost} 💎\nYou have: ${user.gems || 0} 💎\n\n_Try !hospital heal (Ryo) or !rest (free) instead._`
                        });
                    }
                    user.gems -= gemCost;
                    const healed2 = executeTreatment(user);
                    await user.save();
                    return await conn.sendMessage(from, {
                        text: `🏥 *EMERGENCY SURGERY COMPLETE!*\n\n💚 Fully healed! +${healed2} HP\n❤️ ${user.hp.current}/${user.hp.max} HP\n💎 -${gemCost} Gems\n\n_Tsunade-level medical ninjutsu at work!_`
                    });
                }

                // ── Default: show hospital menu ─────────────────────────────
                return await conn.sendMessage(from, prepareImagePayload(user.character, hospitalText(user)));
            }

            // ════════════════════════════════════════════════════════════════
            // REST — passive free HP recovery over time
            // ════════════════════════════════════════════════════════════════
            case 'rest': {
                if (!user || user.registrationStep !== 'COMPLETED') {
                    return await conn.sendMessage(from, { text: '❌ *Not registered yet!*\n\nType *!start* to begin! 🥷' });
                ...
                if (activeFights.has(senderJid)) {
                    return await conn.sendMessage(from, { text: '⚔️ Can\'t rest mid-battle! Finish or !flee first.' });
                }
                ...
                if (sub === 'status') {
                    ...
                    return await conn.sendMessage(from, { text: restProgressText(status) });
                }

                // ── !rest collect — wake up and bank HP gained ──────────────
                if (sub === 'collect' || sub === 'wake' || sub === 'wakeup') {
                    const status2 = getRestStatus(user);
                    if (!status2.resting) {
                        return await conn.sendMessage(from, { text: `🛌 You're not resting right now.\n\n_!rest — start resting_` });
                    }
                    const result = collectRest(user);
                    await user.save();
                    return await conn.sendMessage(from, { text: collectRestText(result, user.username) });
                }

                // ── Default: start resting (or show status if already resting) ──
                const currentStatus = getRestStatus(user);
                if (currentStatus.resting) {
                    return await conn.sendMessage(from, { text: restProgressText(currentStatus) });
                }

                const { success, msg, session } = startRest(user);
                if (!success) return await conn.sendMessage(from, { text: msg });

                user.restSession = session;
                await user.save();

                const missing = user.hp.max - user.hp.current;
                const hoursToFull = Math.ceil(missing / REST_HP_PER_HOUR);

                return await conn.sendMessage(from, prepareImagePayload(user.character,
                    `🛌 *${user.username} begins resting...*\n\n` +
                    `❤️ ${user.hp.current}/${user.hp.max} HP\n` +
                    `📈 Recovering ${REST_HP_PER_HOUR} HP/hour\n` +
                    `⏰ Full recovery in ~${hoursToFull}h\n\n` +
                    `_!rest status — check progress_\n` +
                    `_!rest collect — wake up early and keep what's regenerated_\n\n` +
                    `_Need it instant? !hospital heal (Ryo) or !hospital surgery (Gems)_`
                ));
            }


            // ════════════════════════════════════════════════════════════════
            // SECRET TECHNIQUE — train a permanent fused jutsu
            // ════════════════════════════════════════════════════════════════
            case 'technique':
            case 'secrettechnique': {
                ...
                // ── !technique status ─────────────────────────────────────
                if (sub === 'status') {
                    const status = getTechniqueTrainingStatus(user);
                    return await conn.sendMessage(from, { text: techStatusText(status) });
                }
                ...
                if (sub === 'collect') {
                    const status = getTechniqueTrainingStatus(user);
                    if (!status.training) {
                        return await conn.sendMessage(from, { text: `🌀 No technique training in progress.\n\n_!technique fuse [jutsu1] | [jutsu2] — start one_` });
                    }
                    if (!status.complete) {
                        return await conn.sendMessage(from, { text: techStatusText(status) });
                    }
                    ...
                    const jutsuA = char.jutsus.find(j => j.id === s.jutsuAId) || { id: s.jutsuAId, name: s.jutsuAName, damage: 100, cost: 50, nature: null };
                    const jutsuB = char.jutsus.find(j => j.id === s.jutsuBId) || { id: s.jutsuBId, name: s.jutsuBName, damage: 100, cost: 50, nature: null };
                    const technique = generateSecretTechnique(jutsuA, jutsuB, s.customName);

                    user.secretTechnique = technique;
                    user.secretTechTraining = { active: false, jutsuAId: '', jutsuBId: '', jutsuAName: '', jutsuBName: '', customName: '', startedAt: null, endsAt: null };
                    await user.save();

                    return await conn.sendMessage(from, prepareImagePayload(user.character, collectedText(technique)));
                }

                // ── !technique fuse [jutsu1] | [jutsu2] ───────────────────
                if (sub === 'fuse') {
                    if (user.secretTechTraining?.active) {
                        return await conn.sendMessage(from, { text: `❌ Already training a technique! Check progress: !technique status` });
                    }

                    const fullArgs = rawText.replace(/^fuse\s+/i, '');
                    const parts = fullArgs.split('|').map(s => s.trim());
                    if (parts.length < 2 || !parts[0] || !parts[1]) {
                        return await conn.sendMessage(from, {
                            text: `🌀 *Usage:* !technique fuse [jutsu1] | [jutsu2]\n\n` +
                                  `Example: !technique fuse Fireball Jutsu | Wind Release\n\n` +
                                  `_!jutsus — see your available jutsus to fuse_`
                        });
                    }

                    const findJutsu = (name) => char.jutsus.find(j => j.name.toLowerCase().includes(name.toLowerCase()));
                    const jutsuA = findJutsu(parts[0]);
                    const jutsuB = findJutsu(parts[1]);

                    if (!jutsuA) return await conn.sendMessage(from, { text: `❌ Jutsu not found: "${parts[0]}"\n\n_!jutsus — see your list_` });
                    if (!jutsuB) return await conn.sendMessage(from, { text: `❌ Jutsu not found: "${parts[1]}"\n\n_!jutsus — see your list_` });
                    if (jutsuA.id === jutsuB.id) return await conn.sendMessage(from, { text: `❌ Pick two DIFFERENT jutsus to fuse.` });

                    const { success, msg, ryoCost, hours, session } = startTechniqueTraining(user, jutsuA, jutsuB, null);
                    if (!success) return await conn.sendMessage(from, { text: msg });

                    user.ryo -= ryoCost;
                    user.secretTechTraining = session;
                    await user.save();

                    const synergy = getSynergy(jutsuA.nature, jutsuB.nature);
                    return await conn.sendMessage(from, prepareImagePayload(user.character,
                        trainingStartedText(session, ryoCost, hours, synergy)
                    ));
                }
                ...
                if (sub === 'cancel') {
                    if (!user.secretTechTraining?.active) {
                        return await conn.sendMessage(from, { text: `🌀 No active training to cancel.` });
                    }
                    user.secretTechTraining = { active: false, jutsuAId: '', jutsuBId: '', jutsuAName: '', jutsuBName: '', customName: '', startedAt: null, endsAt: null };
                    await user.save();
                    return await conn.sendMessage(from, { text: `🌀 Training cancelled. _No Ryo refund — the attempt still cost resources._` });
                }

                // ── Default: show guide / current technique ───────────────
                return await conn.sendMessage(from, prepareImagePayload(user.character,
                    techniqueGuideText(char, user.secretTechnique)
                ));
            }


            // ════════════════════════════════════════════════════════════════
            // GUIDE — full new-player walkthrough
            // ════════════════════════════════════════════════════════════════
            case 'guide':
            case 'howtoplay':
            case 'tutorial': {
                ...
                if (sub === 'combat' || sub === 'battle') {
                    return await conn.sendMessage(from, {
                        text: `⚔️ *GUIDE: COMBAT* ⚔️\n━━━━━━━━━━━━━━━━━━━━\n\n` +
                              `*1. Start a fight*\n   !battle d → !battle s (D=easy, S=legendary)\n   !duel bot — practice, no risk\n   !duel [player] — challenge someone\n\n` +
                              `*2. Pick your move each round*\n   !use — see ALL your options (jutsus, weapon, dojutsu, beast)\n   !use [1-3] — cast a jutsu\n   !use 0 — free basic strike\n   !use secret — your trained Secret Technique\n\n` +
                              `*3. Special abilities (if you have them)*\n   !weaponjutsu — your equipped weapon's move\n   !dojutsu — eye technique moves\n   !beast — Tailed Beast Chakra Mode\n   !bijuumode — Stage 3 ultimate (Jinchuriki only)\n\n` +
                              `*4. End the fight*\n   Win = Ryo + XP. Run low on HP? !flee to escape.\n\n_!guide — back to main guide_`
                    });
                }

                if (sub === 'progress' || sub === 'progression') {
                    return await conn.sendMessage(from, {
                        text: `📈 *GUIDE: GETTING STRONGER* 📈\n━━━━━━━━━━━━━━━━━━━━\n\n` +
                              `*Jutsus:* !jutsus to view, !buyjutsu to unlock, !equip to set active\n` +
                              `*Weapons:* !weaponshop to browse, !buyweapon, !equipweapon\n` +
                              `*Dojutsu:* automatic if your character has one (Sasuke/Itachi/Neji/Pain etc.) — !dojutsu for info\n` +
                              `*Tailed Beast:* automatic if Jinchuriki (Naruto/Gaara/Killer Bee) — !beastinfo for info\n` +
                              `*Secret Technique:* !technique — fuse 2 jutsus into a permanent custom move\n` +
                              `*Training Ground:* !trainingground — passive XP/Ryo over real time, no chakra cost\n` +
                              `*Hospital/Rest:* !hospital (pay) or !rest (free, slow) to heal HP\n\n` +
                              `_!guide — back to main guide_`
                    });
                }

                if (sub === 'social' || sub === 'group') {
                    return await conn.sendMessage(from, {
                        text: `🏯 *GUIDE: SOCIAL & GROUP FEATURES* 🏯\n━━━━━━━━━━━━━━━━━━━━\n\n` +
                              `*Clans:* !clans to browse, !joinclan to apply\n` +
                              `*Akatsuki:* !akatsuki info — an alternative dark path (Chunin+, Lv.30+)\n` +
                              `*Village War:* !war — Kage-led conflicts between villages\n` +
                              `*Boss Raids:* !raid info — multiplayer boss fights (group only)\n` +
                              `*Chunin Exam:* !exam info — tournament bracket vs other players\n` +
                              `*Gifts:* !gift [player] [amount] — send Ryo to a friend\n` +
                              `*Arena Season:* !arena — your rank, !season for leaderboard\n\n` +
                              `_!guide — back to main guide_`
                    });
                }
                ...
                // ── Main guide ──────────────────────────────────────────────
                return await conn.sendMessage(from, {
                    text: `🍥 *NARUTO RPG — NEW PLAYER GUIDE* 🍥\n━━━━━━━━━━━━━━━━━━━━\n\n` +
                          `Welcome, shinobi! This bot has a LOT of features — here's how it all fits together.\n\n` +
                          `*🚀 Quick Start:*\n` +
                          `1️⃣ !start — create your ninja (if you haven't)\n` +
                          `2️⃣ !profile — see your full stats\n` +
                          `3️⃣ !battle d — fight your first enemy\n` +
                          `4️⃣ !daily — claim your daily bonus\n\n` +
                          `━━━━━━━━━━━━━━━━━━━━\n` +
                          `*📖 Detailed Guides:*\n\n` +
                          `⚔️ *!guide combat* — how battles & moves work\n` +
                          `📈 *!guide progress* — jutsus, weapons, dojutsu, beasts, training\n` +
                          `🏯 *!guide social* — clans, war, raids, exams, gifts\n\n` +
                          `━━━━━━━━━━━━━━━━━━━━\n` +
                          `📋 *!menu* — full command list (3 pages)\n` +
                          `📣 *!killfeed* — what gets announced automatically\n\n` +
                          `_Lost mid-battle? Just type !use with no number — it shows everything you can do that round._`
                });
            }
