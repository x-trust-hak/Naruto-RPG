    ...
    try {
        ...
        switch (command) {
            ...
            case 'profile': {
                ...
                const profCard = await renderProfileCard({
                    ...
                    wins: user.wins || 0, losses: user.losses || 0,
                    rankPoints: user.rankPoints || 0,
                    attack: ps.attack, defense: ps.defense, speed: ps.speed, crit: ps.crit,
                    imageName: getCharacterImage(user.character), emoji: ps.emoji,
                    clanName: user.clan !== 'None' ? user.clan : null,
                }).catch(() => null);

                await conn.sendMessage(from,
                    profCard
                        ? prepareImagePayloadFromBuffer(profCard, profileMsg)
            ...
            case 'shop': {
                ...
                if (!shopArg || shopArg === 'chars') {
                    await conn.sendMessage(from, {
                        text: `🛒 *NINJA CHARACTER SHOP*\n\n` +
                              `💰 You have *${(user.ryo || 0).toLocaleString()} Ryo* | 💎 *${user.gems} Gems*\n\n` +
                              `_Tap a card below to buy or switch characters. You own: ${user.ownedCharacters.length} ninja(s)._`
                    });

                    for (const char of Object.values(CHARACTERS)) {
                        const owned = user.ownedCharacters.includes(char.id);
                        const imageKey = getCharacterImage(char.id);
                        const imagePath = require('path').join(__dirname, '../images', imageKey + '.jpg');
                        let imageBuffer = null;
                        try { imageBuffer = require('fs').readFileSync(imagePath); } catch (_) {}

                        const price = owned ? '✅ Already owned' : `💰 ${char.price.toLocaleString()} Ryo`;
                        const nature = char.nature ? ` · ${natureTag(char.nature)}` : '';
                        const caption =
                            `${char.emoji} *${char.name}*\n` +
                            `${char.rarity}${nature} · ${char.village}\n` +
                            `${price}\n\n` +
                            `_${char.description}_`;

                        const btnLabel = owned ? `✅ Switch to ${char.name}` : `💰 Buy ${char.name}`;
                        const btnId = owned ? `!switch ${char.name}` : `!buy ${char.name}`;

                        if (imageBuffer) {
                            await conn.sendMessage(from, { image: imageBuffer, caption, mimetype: 'image/jpeg' });
                        } else {
                            await conn.sendMessage(from, { text: caption });
                        }
                        await conn.sendMessage(from, buttonMessage(
                            `Tap to ${owned ? 'equip' : 'buy'} ${char.name}!`,
                            [{ label: btnLabel, id: btnId }]
                        ));
                    }
                    return;
                }
