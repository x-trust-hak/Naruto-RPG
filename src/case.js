        ...
        switch (command) {
            ...
            case 'fight': {
                ...
                user.equippedJutsus.forEach((id, i) => {
                    ...
                    if (j) moveList += `  *${i + 1}.* ${j.name}${j.nature ? ` ${natureTag(j.nature)}` : ''} (💧${j.cost})\n`;
                });
            ...
            case 'attack': {
                ...
                // Player's action
                const pHit = resolveHit(ps, enemyCombatant, jutsu.damage, jutsu.nature);
                if (jutsu.damage < 0) {
                    ...
                    log += `${ps.emoji} You use *${jutsu.name}* — ${pHit.crit ? '💥CRIT ' : ''}${pHit.dealt} dmg${pHit.natureLabel ? ` ${pHit.natureLabel}` : ''}${pHit.healed ? ` (🩸+${pHit.healed})` : ''}!\n`;
                }
            ...
            case 'item': {
                ...
                const buffs = await getClanBuffs(user);
                const ps = getEffectiveStats(user, buffs);
                const enemyCombatant = { attack: fight.enemyStats.attack, defense: fight.enemyStats.defense, crit: fight.enemyStats.crit, dodge: 0, lifesteal: 0, nature: fight.enemyNature };
                const eMove = enemyChooseMove(fight, fight.enemyHp, fight.enemyMaxHp);
                const eHit = resolveHit(enemyCombatant, ps, eMove.damage, eMove.nature);
                if (eMove.damage < 0) {
                    fight.enemyHp = Math.min(fight.enemyMaxHp, fight.enemyHp + eHit.healed);
