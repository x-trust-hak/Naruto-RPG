        ...
        switch (command) {
            ...
            case 'attack': {
                ...
                if (fight.type === 'pvp') {
                    ...
                    const p2User = await User.findOne({ phoneId: fight.p2.jid });
            ...
            case 'accept': {
                ...
                const p1Pp = {}; (challenger.jutsus || []).forEach(id => { p1Pp[id] = CHARACTERS[challenger.character]?.jutsus.find(j => j.id === id)?.pp ?? 99; });
                const p2Pp = {}; (user.jutsus || []).forEach(id => { p2Pp[id] = CHARACTERS[user.character]?.jutsus.find(j => j.id === id)?.pp ?? 99; });
