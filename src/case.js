        ...
        switch (command) {
            ...
            case 'fight': {
                ...
                user.equippedJutsus.forEach((id, i) => {
                    ...
                    if (j) moveList += `  *${i + 1}.* ${j.name} (💧${j.cost})\n`;
                });
            ...
            case 'attack': {
                ...
                const jutsuId = user.equippedJutsus[idx];
                const jutsu = jutsuId ? char.jutsus.find(j => j.id === jutsuId) : null;
                if (!jutsu) {
                    return await conn.sendMessage(from, { text: `❌ Invalid move. You have ${user.equippedJutsus.length} jutsu(s). Use !use 1` });
                }
                if (user.chakra.current < jutsu.cost) {
                    return await conn.sendMessage(from, { text: `❌ Not enough chakra for *${jutsu.name}* (need ${jutsu.cost}, have ${user.chakra.current}).\nTry a cheaper jutsu or !item soldier pill.` });
                }
                ...
                // Player's action
                const pHit = resolveHit(ps, enemyCombatant, jutsu.damage);
                if (jutsu.damage < 0) {
                    user.hp.current = Math.min(user.hp.max, user.hp.current + pHit.healed);
                    log += `${ps.emoji} You use *${jutsu.name}* — 💚 healed ${pHit.healed} HP!\n`;
                } else if (pHit.dodged) {
                    log += `${ps.emoji} You use *${jutsu.name}* — 👻 ${fight.enemyName} dodged!\n`;
                } else {
                    fight.enemyHp -= pHit.dealt;
                    if (pHit.healed) user.hp.current = Math.min(user.hp.max, user.hp.current + pHit.healed);
                    log += `${ps.emoji} You use *${jutsu.name}* — ${pHit.crit ? '💥CRIT ' : ''}${pHit.dealt} dmg${pHit.healed ? ` (🩸+${pHit.healed})` : ''}!\n`;
                }
                ...
                // Enemy's action
                const eMove = enemyChooseMove(fight, fight.enemyHp, fight.enemyMaxHp);
                const eHit = resolveHit(enemyCombatant, ps, eMove.damage);
                if (eMove.damage < 0) {
                    ...
                    log += `${fight.enemyEmoji} ${fight.enemyName} uses _${eMove.name}_ — ${eHit.crit ? '💥CRIT ' : ''}${eHit.dealt} dmg!\n`;
                }
            ...
            case 'item': {
                ...
                if (!item || item.category !== 'consumable') {
                    return await conn.sendMessage(from, { text: `❌ You can only use consumables in battle.` });
                }
                ...
                if (item.effect.fullHeal) {
                    ...
                    if (item.effect.heal)   { const b = user.hp.current; user.hp.current = Math.min(user.hp.max, user.hp.current + item.effect.heal); log += `${item.emoji} You use *${item.name}* — 💚 +${user.hp.current - b} HP\n`; }
                ...
                const buffs = await getClanBuffs(user);
                const ps = getEffectiveStats(user, buffs);
                const enemyCombatant = { attack: fight.enemyStats.attack, defense: fight.enemyStats.defense, crit: fight.enemyStats.crit, dodge: 0, lifesteal: 0 };
                const eMove = enemyChooseMove(fight, fight.enemyHp, fight.enemyMaxHp);
                const eHit = resolveHit(enemyCombatant, ps, eMove.damage);
                if (eMove.damage < 0) {
                    fight.enemyHp = Math.min(fight.enemyMaxHp, fight.enemyHp + eHit.healed);
            ...
            case 'buyitem': {
                ...
                const currency = item.currency || 'ryo';
            ...
            case 'useitem': {
                ...
                if (item.category !== 'consumable') {
                    return await conn.sendMessage(from, { text: `❌ *${item.name}* can't be used directly. (Banners found clans, charms trigger on battle wins.)` });
                }

                let msg = '';
                if (item.effect.heal) {
                    const before = user.hp.current;
                    user.hp.current = Math.min(user.hp.max, user.hp.current + item.effect.heal);
