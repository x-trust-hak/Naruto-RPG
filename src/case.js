...
const { initComboState, setComboSetup, checkComboFinish, comboLogLine, comboSetupHint, comboGuideText } = require('./jutsuCombo');
        ...
        switch (command) {
            ...
            case 'attack': {
                ...
                if (moveBuff) {
                    ...
                    // Check for combo finish BEFORE resolving the hit
                    const comboResult = !isBasic ? checkComboFinish(fight, moveName, moveNature, fight.round) : null;
                    const comboMult = comboResult ? comboResult.mult : 1;
                    const effectiveDamage = Math.round(moveDamage * comboMult);

                    // Player's action
                    const pHit = resolveHit(ps, enemyCombatant, effectiveDamage, moveNature);
                    if (moveDamage < 0) {
                    ...
                    // Combo feedback
                    if (comboResult) {
                        log += comboLogLine(comboResult);
                    } else if (!isBasic && moveDamage > 0) {
                        // This jutsu becomes the new combo setup for next turn
                        setComboSetup(fight, moveName, moveNature, fight.round);
                        log += comboSetupHint(moveName);
                    }
                }
                ...
                if (fight.comboState?.setupMove && (fight.round - fight.comboState.setupMove.round) <= 1) {
                    modeTags.push(`🔗 COMBO READY (chain it now!)`);
                }
                        ...
                        sections: [
                            ...
                            },
                            {
                                ...
                                rows: [
                                    ...
                                    { id: '!combo',     title: '!combo',              description: '🔗 Jutsu Combo guide — chain 2 jutsus for bonus damage' },
                                ]
                            },
                            {
            ...
            case 'eye': {
                ...
                if (move.damage > 0) {
                    ...
                    // Combo check — dojutsu techniques can chain too
                    const dojComboResult = checkComboFinish(fight, move.name, move.nature, fight.round);
                    const dojComboMult = dojComboResult ? dojComboResult.mult : 1;
                    const dojEffectiveDamage = Math.round(move.damage * dojComboMult);

                    const hit = resolveHit(
                        psD, enemyCombatant2, dojEffectiveDamage, move.nature,
                    ...
                    if (dojComboResult) {
                        log += comboLogLine(dojComboResult);
                    } else {
                        setComboSetup(fight, move.name, move.nature, fight.round);
                        log += comboSetupHint(move.name);
                    }
            ...
            case 'wj': {
                ...
                // Check for combo finish — weapon jutsus can both set up and finish combos
                const wjComboResult = checkComboFinish(fight, wj.name, wj.nature, fight.round);
                const wjComboMult = wjComboResult ? wjComboResult.mult : 1;
                const wjEffectiveDamage = Math.round(wj.damage * wjComboMult);

                const hit = resolveHit(ps, enemyCombatant, wjEffectiveDamage, wj.nature,
                ...
                // Combo feedback
                if (wjComboResult) {
                    log += comboLogLine(wjComboResult);
                } else {
                    setComboSetup(fight, wj.name, wj.nature, fight.round);
                    log += comboSetupHint(wj.name);
                }
            ...
            // ════════════════════════════════════════════════════════════════
            // JUTSU COMBO — info guide
            // ════════════════════════════════════════════════════════════════
            case 'combo':
            case 'combos': {
                // If in battle, show current combo window status
                if (activeFights.has(senderJid)) {
                    const fight = activeFights.get(senderJid);
                    if (fight.comboState?.setupMove) {
                        const setup = fight.comboState.setupMove;
                        const roundsAgo = fight.round - setup.round;
                        if (roundsAgo <= 1) {
                            return await conn.sendMessage(from, {
                                text: `🔗 *COMBO WINDOW OPEN!*\n\n` +
                                      `Setup move: *${setup.name}*${setup.nature ? ` (${natureTag(setup.nature)})` : ''}\n\n` +
                                      `⚡ *Cast another jutsu THIS turn to chain the combo!*\n` +
                                      `_!use [number] / !weaponjutsu / !dojutsu [n]_`
                            });
                        }
                    }
                    return await conn.sendMessage(from, {
                        text: `🔗 No combo window active.\n\n_Cast any jutsu to start one — your next jutsu can chain off it!_`
                    });
                }
                return await conn.sendMessage(from, { text: comboGuideText() });
            }
