...
// `side` = 'self' is whoever cast the jutsu (the enemy/monster side is never
// the caster in PvE since monsters carry no buff field, but this stays
// symmetric so it also works for PvP later).
// Returns a log line (may be empty string).
function applyBuff(effects, side, buff, casterName) {
    if (!buff) return '';
    const other = side === 'self' ? 'enemy' : 'self';
    let line = '';

    if (buff.stun) {
        effects[other].stun += buff.stun;
