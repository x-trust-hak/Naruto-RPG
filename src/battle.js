...
// baseDamage > 0 = damage, < 0 = heal (abs). Returns { dealt, healed, crit, dodged }.
function resolveHit(attacker, defender, baseDamage) {
    // Heal move
    if (baseDamage < 0) {
        return { dealt: 0, healed: Math.abs(baseDamage), crit: false, dodged: false };
    }

    // Dodge check
    if (Math.random() * 100 < (defender.dodge || 0)) {
        return { dealt: 0, healed: 0, crit: false, dodged: true };
    }
    ...
    return { dealt: dmg, healed, crit, dodged: false };
}
...
function simulateDuel(s1, s2) {
    ...
    for (let round = 1; round <= 20; round++) {
        ...
        for (const turn of order) {
            ...
            let baseDmg, label, cost;

            if (jutsu) {
                baseDmg = jutsu.damage; label = jutsu.name; cost = jutsu.cost;
                atkCh -= cost;
            } else {
                baseDmg = 40; label = 'Taijutsu Strike'; cost = 0;
            }

            // Buff-only jutsus (damage 0, has buff) -> treat as small chip + flavor
            if (baseDmg === 0) baseDmg = 30;

            const res = resolveHit(atk, def, baseDmg);
            ...
            if (res.healed && baseDmg < 0) {
                ...
                log.push(`${atk.emoji} *${atk.name}* uses _${label}_ — ${res.crit ? '💥CRIT ' : ''}${res.dealt} dmg${res.healed ? ` (🩸+${res.healed})` : ''}`);
            }
